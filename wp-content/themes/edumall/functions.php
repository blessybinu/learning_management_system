...
add_filter('template_include','filter_template_properties', 99);
/**
 * Init the theme
 */
Edumall_Init::instance()->initialize();

function filter_template_properties( $template ){
	$page_name = get_query_var('pagename');
	if ($page_name == 'woo_order') {
		$custom_template = EDUMALL_WOO_TEMPLATE.'/woo_order.php';

		if ( $custom_template ) {
			return $custom_template;
		}
	}
	else if($page_name == 'create_order') {
		$custom_template = EDUMALL_WOO_TEMPLATE.'/create_order.php';
